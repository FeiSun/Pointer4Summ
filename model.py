...
class SummarizationModel(object):
    ...
    def _add_decoder(self, inputs):
        ...
        outputs, out_state, attn_dists, coverage = attention_decoder(
            inputs,
            self._dec_in_state,
            self._enc_states,
    ...
    def _calc_final_dist(self, attn_dists):
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('decoder'):
                decoder_outputs, self._dec_out_state, self.attn_dists, self.coverage = self._add_decoder(
                    emb_dec_inputs)

            # For pointer model, calc final distribution from copy distribution
            final_dists = self._calc_final_dist(self.attn_dists)
        ...
        with tf.variable_scope('loss'):
            if FLAGS.pointer_gen:
                ...
                for dec_step, dist in enumerate(self.final_dists):
                    ...
                    indices = tf.stack(
                        (batch_nums, targets), axis=1)  # shape (batch_size, 2)
    ...
    """
        ...
        (enc_states, dec_in_state, global_step) = sess.run(
            [self._enc_states, self._dec_in_state, self.global_step],
    ...
    def decode_onestep(self, sess, batch, latest_tokens, enc_states,
        ...
        return results['ids'], results[
            'probs'], new_states, attn_dists, new_coverage
...
def _coverage_loss(attn_dists, padding_mask):
  ...
  """
    coverage = tf.zeros_like(attn_dists[
        0])  # shape (batch_size, attn_length). Initial coverage is zero.
    covlosses = [
